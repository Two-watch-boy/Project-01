...
function bagsSuccess(json) {
  $('.addBag').on('mousedown', function(){
    console.log("clicked for bag");
    $('.bag0').append('<div class="col-sm-offset-3 col-sm-6 box">' + '<img class="glyph"src="/images/glyphicons-342-briefcase.png">' + '<p>' + json[i].type + '</p>' + '<img class="addItem button" src="/images/glyphicons-191-plus-sign.png">' + '</div>');
...
function addItemListener(json){
  $('.addItem').on('mousedown', function(){
    ...
    $('.bag0').append('<div class="col-xs-offset-4 col-sm-5 space">' + json[i].contents[o].item + ' </div>');
    o++;
